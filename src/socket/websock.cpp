...
WebSock::WebSock(int domain, int type, int protocol, bool ssl_socket,
                 bool server)
  ...
  if (ssl_socket) {
    ...
    _ssl_socket->init(server ? ".ssl/server.crt" : "",
...
SSLWebSock *WebSock::get_ssl() { return _ssl_socket; }
...
bool WebSock::listen(int backlog) { return (::listen(_socket, backlog) == 0); }

int WebSock::accept(sockaddr_in *clientAddr) {
  socklen_t len = clientAddr ? sizeof(*clientAddr) : 0;
  auto client = ::accept(
      _socket, clientAddr ? reinterpret_cast<sockaddr *>(clientAddr) : nullptr,
...
int WebSock::get_socket() { return (int)_socket; }
